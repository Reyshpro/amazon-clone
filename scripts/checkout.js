...
function renderOrderSummary() {
  ...
  cart.forEach((cartItem) => {
    ...
    // Ensure every cart item has a delivery option
    ...
    let dateString = 'No delivery option selected';
...
function addEventListeners() {
  ...
  document.querySelectorAll('.js-delivery-option').forEach((element) => {
    element.addEventListener('click', () => {
      ...
      renderOrderSummary(); // refresh the cart after selection
    });
  });
}
...
const links = document.querySelectorAll('.update-quantity-link');
links.forEach((link) => {
  link.addEventListener('click', () => {
    const productId = link.dataset.productId;
    const container = document.querySelector(`.js-cart-item-container-${productId}`);
...
// Handle Save buttons and Enter key
document.querySelectorAll('.js-save-link').forEach((link) => {
  const productId = link.dataset.productId;
  const input = document.querySelector(`.quantity-input[data-product-id="${productId}"]`);
  const container = document.querySelector(`.js-cart-item-container-${productId}`);
